...
//        starRunner = new Runnable() {
//            @Override
//            public void run() {
//                //
//            }
//        };
        starThread = new newThreadClass(this);
        starThread.start();
    ...
    public void progressChange(int newProgress) {
        ...
        while(field.size() > (numStars+INIT_STAR_COUNT)){
            removeStar();
        }
        while(field.size() < (numStars+INIT_STAR_COUNT)){
            addStar();
        ...
        Star.starPaint.setColor(Color.rgb(222, 222, 222));
        this.twinkle = false;
    }
